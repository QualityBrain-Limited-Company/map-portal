...
export default function DocumentCard({ document: docData }: DocumentCardProps) {
  const [isExpanded, setIsExpanded] = useState(false)
  const setSelectedDocument = useMapStore((state) => state.setSelectedDocument)

  const handleViewOnMap = () => {
    setSelectedDocument(docData)
    const mapElement = window.document.querySelector('#map-container')
  ...
  const handleDownload = async () => {
    try {
      ...
      aElement.href = url
      aElement.download = `${docData.title}.pdf`
      window.document.body.appendChild(aElement)
      aElement.click()
      window.URL.revokeObjectURL(url)
      window.document.body.removeChild(aElement)
    } catch (error) {
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-gray-500">
              {formatDistanceToNow(new Date(docData.createdAt), {
                addSuffix: true,
                locale: th
              })}
            </span>
          ...
          {docData.description.length > 150 && (
            <button
          ...
          <div className="text-sm text-gray-500 mt-2 flex items-center gap-1">
            ...
            <span className="truncate">
              {docData.subdistrict}, {docData.district}, {docData.province}
            </span>
          </div>
          ...
          <div className="flex items-center gap-4 mt-3">
            ...
            <div className="flex items-center text-sm text-gray-500">
              ...
              {docData.viewCount.toLocaleString()}
            </div>
            ...
            <div className="flex items-center text-sm text-gray-500">
              ...
              {docData.downloadCount.toLocaleString()}
            </div>
